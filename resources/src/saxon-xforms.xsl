                    ...
                    <xsl:if test="exists($setvalueVar)">
                        ...
                        <xsl:for-each select="array:flatten($setValues)">
                            ...
                            <xsl:if test="exists($refz)">
                                <xsl:variable name="associated-form-control" select="ixsl:page()//*[@data-ref = $refz]" as="node()?"/>
...
<!-- Only to use this function on simple path cases -->
    ...
    <xsl:function name="xforms:resolveXPathStrings" as="xs:string">
        ...
        <xsl:variable name="slashes" select="if(contains($base, '/')) then index-of(string-to-codepoints($base), string-to-codepoints('/')) else 0" as="xs:integer*"/>
        ...
        <xsl:variable name="parentSlash" as="xs:integer">
            <xsl:choose>
        ...
        <xsl:choose>
            <xsl:when test="$parentCallCount gt 0">
                ...
                <xsl:sequence select="concat(substring($base, 1, $parentSlash), replace($relative, '\.\./', ''))"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="select[exists(@data-action)]" mode="ixsl:onchange">
        ...
        <xsl:variable name="xforms-actions" select="js:getAction(string(@data-action))" as="map(*)*"/>
        
        <xsl:message use-when="$debugMode">action map = <xsl:sequence select="serialize($xforms-actions)"/>
        </xsl:message>
                ...
                <xsl:if test="$ifExecuted">
                    ...
                    <xsl:if test="exists($setvalueVar)">
                        <xsl:variable name="setValues" as="item()*">
                         ...
                         <xsl:message use-when="$debugMode">
                            setValues <xsl:value-of select="serialize(array:flatten($setValues))"/>
                    </xsl:message> 
                        ...
                        <xsl:for-each select="$setValues">
                            ...
                            <xsl:variable name="ifExecutedi" as="xs:boolean">
                                <xsl:choose>
                                    <xsl:when test="exists($ifVari)">
                                    <xsl:evaluate xpath="$ifVari" context-item="$context"/>
                                </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:sequence select="true()"/>
                                    </xsl:otherwise>
                            ...
                            <xsl:message use-when="$debugMode"> ref = <xsl:value-of select="$refz"/>
                        </xsl:message>
    ...
    <xsl:function name="xforms:check-required-fields" as="item()*">
        ...
        <xsl:for-each select="$required-fieldsi">
            ...
            <xsl:variable name="resulti">
                <xsl:evaluate xpath="concat('boolean(normalize-space(', @data-ref, '))', '=', @data-ref, '/', @data-required)" context-item="$updatedInstanceXML"/>
            </xsl:variable>
    ...
    <xsl:function name="xforms:check-constraints-on-fields" as="item()*">
        ...
        <xsl:for-each select="$constraint-fieldsi">
            ...
            <xsl:variable name="resulti" as="xs:boolean">
                <xsl:evaluate xpath="@data-constraint" context-item="$contexti"/>
            </xsl:variable>
    ...
    <xsl:template match="xforms:input">
        ...
        <xsl:message use-when="$debugMode">xforms:input=<xsl:value-of select="serialize($instance1)"/> ref= <xsl:value-of select="@ref"/>, <xsl:value-of select="serialize(.)"/> nodeset = <xsl:value-of select="$nodeset"/> 
        </xsl:message> 
        ...
        <xsl:variable name="refi" as="xs:string">
            <xsl:choose>
                <xsl:when test="exists(@ref)">
                    <xsl:value-of select="                             if ($nodeset = '') then                                 concat($nodeset, @ref)                             else                                 concat($nodeset, '/', @ref)"/>
                </xsl:when>
                <xsl:when test="exists($bindingi)">
                    <xsl:value-of select="                             if (exists($bindingi/@nodeset)) then                                 $bindingi/@nodeset                             else                                 $bindingi/@ref"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="''"/>
                </xsl:otherwise>
            ...
            <xsl:choose>
                <xsl:when test="exists($bindingi) and exists($bindingi/@nodeset)">
                    <xsl:message use-when="$debugMode">
                        ...
                        instanceForBinding found binding/@nodeset <xsl:value-of select="$bindingi/@nodeset"/>
                    </xsl:message>
        ...
        <xsl:variable name="myid" select="             if (exists(@id)) then             @id             else             concat(generate-id(), $position)"/>
        ...
        <xsl:message use-when="$debugMode"> binding relevant <xsl:value-of select="$bindingi/@relevant"/> isRelevantVar = <xsl:value-of select="serialize($relevantVar)"/> refi = <xsl:value-of select="$refi"/>
        </xsl:message>
        ...
        <input>
            ...
            <xsl:if test="exists($bindingi)">
                <xsl:message use-when="$debugMode">binding found! : <xsl:value-of select="serialize($bindingi)"/> refi = <xsl:value-of select="$refi"/>
                    </xsl:message>
            </xsl:if>
            <xsl:choose>
                ...
                <xsl:when test="                         if (exists($bindingi)) then                             xs:QName($bindingi/@type) eq xs:QName('xs:boolean')                         else                             false()">
                    ...
                    <xsl:if test="exists($instance1) and exists($bindingi/@nodeset)">
                        ...
                        <xsl:message use-when="$debugMode">
                                <xsl:value-of select="$bindingi/@nodeset"/>, $checkedi <xsl:value-of select="serialize($checkedi)"/>
                            </xsl:message>
    ...
    <xsl:template match="xforms:textarea" priority="2">
        ...
        <textarea>
            ...
            <xsl:attribute name="data-element" select="$refElement"/>
            <xsl:attribute name="data-ref" select="                     if ($nodeset = '') then                         concat($nodeset, @ref)                     else                         concat($nodeset, '/', @ref)"/>
            <xsl:choose>
    ...
    <xsl:template match="xforms:select1 | xforms:select">
        ...
        <xsl:variable name="refi" select="                 if ($nodeset = '') then                     concat($nodeset, @ref)                 else                     concat($nodeset, '/', @ref)"/>
    ...
    <xsl:template match="*" mode="insert-node">
        ...
        <xsl:variable name="currentPosition" select="count(preceding-sibling::*[local-name() = local-name(current())]) + 1"/>
        <xsl:variable name="isLast" select="count(parent::*/*[local-name() = $name]) = $currentPosition"/>
    ...
    <xsl:template match="button[exists(@data-action)]" mode="ixsl:onclick">
        ...
        <xsl:variable name="action-map" select="js:getAction(string(@data-action))"/>
        ...
        <xsl:message use-when="$debugMode"> instanceXML XXX= <xsl:value-of select="serialize($updatedInstanceXML)"/> 
        </xsl:message>
            ...
            <xsl:choose>
                <xsl:when test=". = 'insert'">
                    <xsl:variable name="inserts" as="item()*">
                        <xsl:sequence select="array:flatten(map:get($action-map, 'insert'))"/>
                    </xsl:variable>
                    ...
                    <xsl:for-each select="array:flatten($inserts)">
                        ...
                        <xsl:variable name="insert-node" as="node()">
                            <xsl:evaluate xpath="concat($map-ref,'[','last()',']')" context-item="$updatedInstanceXML"/>
                        </xsl:variable>
                        <xsl:message use-when="$debugMode">insert found !!!! insert = <xsl:value-of select="serialize(map:get($action-map, 'insert'))"/> ref = <xsl:value-of select="$map-ref"/>
                        </xsl:message>
                ...
                <xsl:when test=". = 'delete'">
                    <xsl:variable name="deletes" as="item()*">
                        <xsl:sequence select="array:flatten(map:get($action-map, 'delete'))"/>
                    </xsl:variable>
                    ...
                    <xsl:for-each select="$deletes">
                        ...
                        <xsl:variable name="if-statement" as="xs:boolean">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:evaluate xpath="$if-clause" context-item="$updatedInstanceXML"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        ...
                        <xsl:if test="$if-statement">
                            <xsl:variable name="instance-with-delete">
                                ...
                                <xsl:apply-templates select="$updatedInstanceXML" mode="delete-node">
                                    <xsl:with-param name="path" select="$deleteRefFixed"/>
                                </xsl:apply-templates>
...
<!--
    ...
    <xsl:template match="xforms:action">
        ...
        <xsl:param name="position" select="''"/>
        <xsl:param name="nodeset" select="''"/>
        <xsl:param name="myid" tunnel="yes" select="             if (exists(@id)) then             @id             else             concat(generate-id(), $position)"/>
        <xsl:variable name="time-id" select="generate-id($instance1)"/>
        ...
        <xsl:variable name="action-map" as="map(*)">
            <xsl:map>
                    ...
                    <xsl:for-each-group select="xforms:setvalue | xforms:insert | xforms:delete | xforms:toggle | xforms:send | xforms:setfocus | xforms:setindex | xforms:load | xforms:message | xforms:dispatch | xforms:rebuild | xforms:reset | xforms:show | xforms:hide | xforms:script | xforms:unload" group-by="local-name()">
    ...
    <xsl:template match="xforms:setvalue | xforms:insert | xforms:delete | xforms:toggle | xforms:send | xforms:setfocus | xforms:setindex | xforms:load | xforms:message | xforms:dispatch | xforms:rebuild | xforms:reset | xforms:show | xforms:hide | xforms:script | xforms:unload">
        <xsl:param name="instance1" as="node()?" select="()"/>
        <xsl:param name="bindings" as="map(xs:string, node())" select="map{}"/>
        <xsl:param name="submissions" as="map(xs:string, xs:string)" select="map{}"/>
        <xsl:param name="position" select="''"/>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="current-group()[self::attribute()]">
                            ...
                            <xsl:element name="string" namespace="http://www.w3.org/2005/xpath-functions">
                                <xsl:attribute name="key" select="concat('@', current-grouping-key())"/>
                                <xsl:value-of select="."/>
                            </xsl:element>
    ...
    <xsl:template match="*" mode="form-check">
       ...
       <!-- <xsl:message use-when="$debugMode">form-check processing pendingUpdat Map size: <xsl:value-of select="map:size($pendingUpdates)"/>
        ...
        <xsl:for-each-group select="element()" group-by="local-name(.)">                  
            ...
            <xsl:variable name="dataRefWithFilter" select="ixsl:page()//*[starts-with(@data-ref, concat($updatedChildPath,'['))]"/>
